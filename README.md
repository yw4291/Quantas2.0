...
Introduction
-------------------
Quantas2 is a Snakemake-based workflow for quantifying RNA splicing from raw FASTQ sequencing files, featuring dedicated sub-workflows for  `splicing level quantification` and `RNA expression level quantification (RPKM)` and `splice site usage (SSU)` analysis,etc. This workflow builds upon [Quantas1.0.9](https://zhanglab.c2b2.columbia.edu/index.php/Quantas_Documentation)[1] in Chaolin Zhang lab.
...
| `Quantas_SSU` |Quantification of the normalized 5' and 3' Splicing Site Usage(SSU,value ranges from 0 to 1) in each gene; <br>`Sample`is the name of each iput FASTQ file |Reference genome `fasta` file; <br> Per-sample `fastq` files |  Reads aligned file:`Sample.Aligned.out.sam` <br>  Sam file to bed file:`Sample.bed` <br> Coverage and statistics of mapped reads:`Sample.mapping_stats.txt` <br> Splice site usage of each splice site:`.splice_site_counts.txt` <br> Statistics of splice site usage in each gene:`sumarry.txt` <br>Snakemake report (optional) 
